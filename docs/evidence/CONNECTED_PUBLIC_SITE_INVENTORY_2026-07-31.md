...
| `OBBBA.us` / `obbba.us` | Short brand/entry domain whose final job remains a Product decision | Local site folder exists but has no active nested Git/Astro implementation established by this inventory | HTTPS returns Cloudflare HTTP 403 | No DiscussionBridge binding verified | Provider/DNS state and last known usable response are unverified | Decide redirect, canonical-site, protected-site, or retirement job; identify exact provider and source before any change |
...
| `repealobbbapledge.us` | Pledge/campaign action surface | Local site container has no nested Git implementation; active source mapping unresolved | Apex redirects to `www`; HTTP 200; Pages project `repealobbbapledge-us-temp-placeholder` | No current DiscussionBridge binding verified | Capture placeholder deployment and DNS/redirect state | Decide product job and canonical source; do not infer a Bridge lane from the placeholder |
...
|---|---|---|---|---|---|---|
| `citizenactivist.network` | Public CitizenActivist site and future Astro-managed publishing surface | Workspace source is under `C:\CodeProjects\Projects\CitizenActivist`; no deployable site package was identified in the bounded scan | HTTPS connection timed out; no current public page accepted | Intended connection to `forum.citizenactivist.network`; no active public-site binding accepted | Unknown until hosting/source ownership is established | Diagnose DNS/hosting/repository ownership read-only, then create a separately approved recovery lane |
| `forum.citizenactivist.network` | Citizen Activist Community forum and bounded multi-target proof destination | Self-hosted Discourse; forum authority remains with its operator | HTTP 200 through nginx | Existing bounded proof target only; not evidence of a general many-to-many administration plane | Server/app backup and deployment rollback unverified | Preserve the live forum; independently inventory exact proof bindings, permissions, embed hosts, and rollback before new writes |
...
3. Plain Astro topic 38 exposes the distinction between Discourse Core
   ...
   remediation closes.
4. `OBBBA.us` is a 403 and lacks an accepted active implementation/source job.
...
6. `repealobbbapledge.us` remains a placeholder with unresolved active source.
7. `citizenactivist.network` did not answer on HTTPS while its forum is live.
